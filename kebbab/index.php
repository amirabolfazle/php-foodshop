...
<head>
    ...
    <title>غذا پزی سگ پز | کباب ها</title>
...
<style>
    *{font-family: Vazir FD, sans-serif}
    ...
    <?php        
        $server='sql102.pcshared.com';
        $username='pcsha_32728517';
        $password='44219173';
        $dbname='product';
                ...
                if (mysqli_num_rows($resault)>0){
                    while ($row=mysqli_fetch_assoc($resault)){
                        ...
                        <div class="col-6">
                            <div class="card">
                                <div class="card-body">
                                    <h4 class="card-header bg-danger text-light">فروش ویژه</h3>
                                    <h2 class="card-title p-3">.'.$row['name'].'</h2>
                                    <img class="card-img-top p-3" src="'.$row['pic'].'" alt="kebab" width="5">
                                    <p class="card-text">'.$row['about'].'</p>
                                    <h6 class="card-footer text-success"><s class="text-danger">'.$row['price'].'</h2>
                                </div>
                            </div>
                        </div>
                        ';
